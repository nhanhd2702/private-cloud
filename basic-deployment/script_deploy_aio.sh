...
echo "#  Please make sure that all the required dependencies        #"
echo "#  are installed and all the necessary files are in place.    #"
echo "#                                                             #"
...
if ! vgdisplay | grep -d "cinder-volumes"; then
	...
	if mount | grep $cinder_volumes_disk; then
        ...
        vgcreate vg_$cinder_volumes_disk /dev/$cinder_volumes_disk

        # Create a logical volume using all the free space on the disk
        lvcreate -l 100%FREE -n lv_$cinder_volumes_disk vg_$cinder_volumes_disk

        # Format the logical volume with the ext4 file system
        mkfs.ext4 /dev/vg_$cinder_volumes_disk/lv_$cinder_volumes_disk

        # Mount the logical volume to a directory
        mkdir /mnt/$cinder_volumes_disk
        mount /dev/vg_$cinder_volumes_disk/lv_$cinder_volumes_disk /mnt/$cinder_volumes_disk
else
